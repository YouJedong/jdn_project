...
import { PopularYoutubeContentDto, PopularNextClassContentDto } from '@/types/content'
...
export async function getPopularFullscoreContents(): Promise<PopularYoutubeContentDto[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/content/yt/popular/PERFORMANCE`, {
    next: { revalidate: 60 }, // 60초 동안 캐시
  });
